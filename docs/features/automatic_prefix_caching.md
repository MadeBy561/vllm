...
## Recurrent models and chat boundaries

For Qwen 3.8 Flash Next and GLM 5.3 Flash, the CUDA V2 runner can retain
recurrent state at exact request boundaries, including boundaries inside a KV
page. This supports text generation with tensor parallelism and fixed-length
MTP speculation.
...
The settings control different parts of caching:
...
The response checkpoint covers committed tokens that the model actually
processed. The final emitted token usually has not been processed, so the next
...
the target hidden state needed to rebuild its last, lookahead-dependent row.
These bundles consume the configured KV-cache budget and are evicted when any
...
LoRA, pipeline/context/data parallelism, KV transfer/offloading, sleep mode,
routed-expert output, and dynamic speculative schedules. These configurations
retain the existing cache behavior. Temporary MTP rollback state remains
available under either retention policy.
