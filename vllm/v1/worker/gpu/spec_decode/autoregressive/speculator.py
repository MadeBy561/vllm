    ...
    def propose(
        ...
        if prefill_batch_desc.cg_mode == CUDAGraphMode.FULL:
            if rebuild_prefill_attn_metadata:
                # FULL graph replay still reads persistent block/slot/query
                # buffers captured by address. Refresh the draft KV layout
                # before replay; otherwise DCP-replicated draft KV can replay
                # against stale target/dummy mappings from capture.
            ...
            # layout matches. DCP-replicated draft KV groups intentionally
            # differ from target DCP-sharded groups, so rebuild the draft
            # metadata against the draft cache layout in that case.
