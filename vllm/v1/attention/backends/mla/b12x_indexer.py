...
class B12xIndexerMetadataBuilder(DeepseekV32IndexerMetadataBuilder):
    ...
    def _split_prefill_chunks(
        ...
        return [
            ...
            for chunk in self._split_indexer_prefill_chunks(
...
class B12xSparseIndexer(nn.Module):
    ...
    def __init__(
        ...
        self.output_physical_slots, self.num_q_heads = (
        ...
        self._max_page_table_width = get_block_table_width(
    ...
    def _caps(self, mode: str, rows: int, width: int):
        return self._module.Caps(
            ...
            output_index_space="physical" if self.output_physical_slots else "logical",
        )
    ...
    def forward(
        ...
        if metadata.prefill is not None:
            for chunk in metadata.prefill.chunks:
                ...
                pages = min(
                    max(1, (int(local) + _INDEX_PAGE_SIZE - 1) // _INDEX_PAGE_SIZE),
                    int(chunk.block_table.shape[1]),
                )
                output, score = (
                    self.topk_indices_buffer[start:end, : self.topk_tokens],
                    scores[start:end] if scores is not None else None,
                )
                _run_paged_topk(
                    module=self._module,
                    plan=self._plan("prefill", int(q.shape[0])),
                    q=q,
                    ...
                    block_table=chunk.block_table[:1, :pages].expand(
                        int(q.shape[0]), pages
                    ),
                    active_width=self.active_width_cap,
                    output=output,
        ...
        if metadata.decode is not None:
            ...
            lengths, tables = (
                decode.seq_lens.reshape(-1).contiguous(),
                decode.block_table,
            )
            if int(tables.shape[0]) != int(lengths.shape[0]):
                ...
                tables = tables.repeat_interleave(
                    int(lengths.shape[0]) // int(tables.shape[0]), dim=0
                )
            rows = metadata.num_decode_tokens
            output, score = (
                self.topk_indices_buffer[:rows, : self.topk_tokens],
                scores[:rows] if scores is not None else None,
            )
            _run_paged_topk(
                module=self._module,
                plan=self._plan("decode", rows),
                q=q_quant[:rows].contiguous(),
                weights=weights[:rows].contiguous(),
                kv_cache=self.k_cache.kv_cache,
                seq_lens=lengths[:rows],
                block_table=tables[:rows].contiguous(),
