...
class B12xPagedAttentionImpl(AttentionImpl[B12xPagedMetadata]):
        ...
        operands = {
            ...
            "k_cache": {
                **self._descriptor(
                ...
                "alignment": min(
                    16, int(key_cache.data_ptr()) & -int(key_cache.data_ptr())
                )
                if key_cache.data_ptr()
                else 16,
            },
            "v_cache": {
                **self._descriptor(
                    value_cache.shape, value_cache.stride(), value_cache.dtype
                ),
                "alignment": min(
                    16, int(value_cache.data_ptr()) & -int(value_cache.data_ptr())
                )
                if value_cache.data_ptr()
                else 16,
            },
            ...
            else self._descriptor(k_descale.shape, k_descale.stride(), k_descale.dtype),
            "v_descale": None
            if v_descale is None
            else self._descriptor(v_descale.shape, v_descale.stride(), v_descale.dtype),
            "attention_sink_bias": None
            if self.sinks is None
            else self._descriptor((self.num_heads,), (1,), torch.float32),
        ...
        for batch in batches:
            ...
            requests.append(
                declaration.request(
                    ...
                    key=key,
                    layer=layer: self._prepared_call(
                        layer, state, key, benchmark=False
                    ),
                    benchmark_call=lambda state,
                    key=key,
                    layer=layer: self._prepared_call(layer, state, key, benchmark=True),
                )
            )
            if self._verify_q_per_req > 1:
                ...
                requests.append(
                    declaration.request(
                        ...
                        key=key,
                        layer=layer: self._prepared_call(
                            layer, state, key, benchmark=True
                        ),
                    )
                )
        ...
        *,
        ...
        if _kv_page_size(key_cache, value_cache) != page_size:
            raise PreparationResourceUnavailableError(
                "b12x paged cache generation changed"
            )
        ...
        *,
        ...
        if attn_metadata.max_query_len <= 1 and total_q == num_reqs:
            ...
            capacity = next(
                (
                    candidate
                    for candidate in self._extend_q_capacities
    ...
    def forward(
        ...
        binding = self._paged_attention.bind(
            ...
            cu_seqlens_q=cu_seqlens_q,
            window_left=self.window_left,
            active_total_q=(None if plan_key[0] == "extend" else num_actual_tokens),
            attention_sink_bias=self.sinks,
            k_descale=k_descale,
            v_descale=v_descale,
        )
