...
class _Linear(nn.Module):
    ...
    def get_b12x_preparation_units(
        ...
        if workload.stage != "weights" or self.weight.is_meta:
            return ()
        self._capacity = (
            workload.max_tokens if self._max_rows is None else self._max_rows
        )
        capacities = tuple(
            sorted(
                {
                    self._capacity,
                    *(rows for rows in workload.token_counts if rows <= self._capacity),
                }
            )
        )
        ...
        for capacity in capacities:
            if capacity not in self._plans:
                self._plans[capacity] = bf16_gemv.plan(
                    bf16_gemv.GemvQuery(
...
class DeepseekV4ViT(nn.Module):
    ...
    def _provision_workspace(self):
        ...
        if self._workspace is None:
            if torch.cuda.is_current_stream_capturing():
                raise RuntimeError(
                    "vision preparation cannot allocate during CUDA graph capture"
                )
    ...
    def get_b12x_preparation_units(
        ...
        if workload.stage != "weights" or any(
            parameter.is_meta for parameter in self.parameters()
        ):
            return ()
        ...
        if self._attention_plan is None:
            ...
            self._attention_plan = varlen.plan(
                ...
                v,
                cu,
                max_seqlen_q=self.capacity,
                max_seqlen_k=self.capacity,
                causal=False,
            )
        ...
        def norm_call(state):
            ...
            source = torch.empty(
                (self.capacity, self.config.vision_dim),
                dtype=torch.bfloat16,
                device=device,
            )
        ...
        def pointwise_call(state):
            ...
            return PreparedCall(
                ...
                if swiglu
                else (lambda: run_add_impl(left, right, plan=state, out=out)),
                produce=produce,
            )
        ...
        requests = (
            ...
            self._norm_plan.request(
                name=f"{base}.norm",
                prepare_call=norm_call,
                benchmark_call=norm_call,
            ),
            self._add_plan.request(
                name=f"{base}.add",
                prepare_call=pointwise_call,
                benchmark_call=pointwise_call,
            ),
    ...
    def forward(self, patches, n_vit_h: int, n_vit_w: int, *, out=None):
        ...
        if (
            ...
            or any(
                plan is None or plan.prepared is None
                for plan in (
                    self._rope_plan,
                    self._attention_plan,
                    self._norm_plan,
                    self._add_plan,
                    self._swiglu_plan,
                )
...
class DeepseekV4Aligner(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.w1 = _Linear(
            config.vision_dim * self.downsample_ratio**2,
            self.out_dim,
            max_rows=capacity,
        )
    ...
    def _provision_workspace(self):
        ...
        if self._workspace is None:
            if torch.cuda.is_current_stream_capturing():
                raise RuntimeError(
                    "aligner preparation cannot allocate during CUDA graph capture"
                )
    ...
    def get_b12x_preparation_units(
        ...
        if self._gelu_plan is None:
            gelu = VisionQuery(
                operation="gelu", channels=self.out_dim, heads=1, ratio=1
            )
        ...
        requests = (
            self._merge_plan.request(
                name=f"{base}.merge",
                prepare_call=merge_call,
                benchmark_call=merge_call,
            ),
            self._gelu_plan.request(
                name=f"{base}.gelu",
                prepare_call=gelu_call,
                benchmark_call=gelu_call,
            ),
        )
        return (
            B12xPreparationUnit(
