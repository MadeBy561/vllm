...
rank has both ranks' Python stacks dumped and is terminated, so a rank-divergent hang
...
def _heartbeat(beats, rank):
    ...
    if _stack_dump_file is not None:
        faulthandler.dump_traceback_later(STALL_SECONDS, file=_stack_dump_file)
...
def _check_linear(kernel, layer, x, weight_fp8):
    ...
    torch.testing.assert_close(
        local.float(),
        expected,
        atol=float(expected.abs().max()) * 0.008 + 1e-6,
        rtol=0.008,
    )
    return local
...
def _reference(local, residual, norm_weight, group):
    ...
    normed = (
        residual_out.float() * torch.rsqrt(variance + EPSILON) * norm_weight.float()
    ).bfloat16()
...
def _build_model(vllm_config, device, rank):
    ...
    values = (
        torch.randn(HIDDEN, HIDDEN, generator=generator, device=device) * 0.05
    ).to(MXFP8_VALUE_DTYPE)
    scales = torch.full(
        (HIDDEN, HIDDEN // 32), 127, dtype=torch.uint8, device=device
...
def _run_rank(rank: int, port: int, beats, fault: str | None, scratch: str) -> None:
    ...
    with open(Path(scratch) / f"rank{rank}.stack", "w") as dump:
        _stack_dump_file = dump
        try:
            ...
            with set_current_vllm_config(vllm_config):
                init_test_distributed_environment(
                    2, 1, rank, str(port), local_rank=rank
                )
...
def _run_rank_body(rank, beats, fault, scratch, device, vllm_config) -> None:
    ...
    try:
        ...
        model, layer, kernel, values, norm_weight = _build_model(
            vllm_config, device, rank
        )
        ...
        worker = SimpleNamespace(
            ...
            model_runner=SimpleNamespace(
                mm_registry=None, _draft_workspace_lane=0, cudagraph_manager=None
            ),
        ...
        winners = {
            count: child.selection.config.to_dict()
        ...
        assert gathered[0] == gathered[1], (
            f"ranks selected different winners: {gathered}"
        )
        if not vllm_config.kernel_config.enable_b12x_autotune:
            assert all(
                child.selection.source == "default" for child in plan.variants.values()
            )
        ...
        for tokens in (MAX_TOKENS, *reversed(SIZES), *UNDECLARED_SIZES):
            ...
            residual = torch.linspace(
                -0.5, 0.5, tokens * HIDDEN, device=device, dtype=torch.bfloat16
            ).view(tokens, HIDDEN)
            ...
            expected, expected_residual = _reference(
                local, residual, norm_weight, tp_group.device_group
            )
            torch.testing.assert_close(out, expected, atol=5e-2, rtol=5e-2)
            torch.testing.assert_close(
                residual_out, expected_residual, atol=2e-2, rtol=2e-2
            )
            _heartbeat(beats, rank)
        ...
        slot_sizes_before = tuple(
            manager._workspace_size_bytes(slot) for slot in manager._current_workspaces
        )
        ...
        with (
            ...
            for tokens in reversed(SIZES):
                ...
                assert elapsed < CAPTURE_STEP_SECONDS, (
                    f"capture of {tokens} tokens took {elapsed:.1f} s"
                )
                ...
                _heartbeat(beats, rank)
        program_keys = sorted(
            f"{program.dialect}:{program.name}" for program in observed
        )
        slot_sizes_after = tuple(
            manager._workspace_size_bytes(slot) for slot in manager._current_workspaces
        )
        assert slot_sizes_after == slot_sizes_before, (
            slot_sizes_before,
            slot_sizes_after,
        )
        ...
        session = (
            getattr(worker, "_b12x_session", None) if "worker" in locals() else None
        )
...
def _fresh_triton_kernel(x: torch.Tensor) -> None:
    ...
    def _oracle_fault_kernel(pointer, count, BLOCK: tl.constexpr):
        ...
        mask = offsets < count
        tl.store(
            pointer + offsets, tl.load(pointer + offsets, mask=mask) + 1, mask=mask
        )
...
def _spawn_with_watchdog(
    ...
    while True:
        ...
        if stale or now > deadline_total:
            ...
            for rank, process in enumerate(procs.processes):
                path = Path(scratch) / f"rank{rank}.stack"
                if process.is_alive() and path.exists():
                    dumps.append(str(path))
            dump_deadline = time.monotonic() + 5
            while any(Path(path).stat().st_size == 0 for path in dumps):
            ...
            if recorded:
                ...
                error = (
                    "\n".join((Path(scratch) / path).read_text() for path in recorded)
                    + f"\n{error}"
                )
            break
...
def test_b12x_tp2_startup_oracle(
    monkeypatch: pytest.MonkeyPatch, tmp_path, autotune
) -> None:
...
def test_b12x_tp2_startup_oracle_faults_fail_within_the_timeout(
    ...
    assert elapsed < STALL_SECONDS * 4 + 120, (
        f"fault {fault!r} took {elapsed:.0f} s to surface"
    )
    if fault == "skip_collective":
        assert len(dumps) == 2, "the watchdog must dump both ranks on a stall"
        assert all("_run_rank_body" in Path(path).read_text() for path in dumps)
