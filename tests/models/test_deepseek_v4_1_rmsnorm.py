...
def test_rmsnorm_prepared_forward_and_replay(monkeypatch, hidden, weight_dtype, eps):
    ...
    layer.weight = torch.nn.Parameter(
        torch.linspace(0.5, 1.5, hidden, device=device, dtype=weight_dtype),
        requires_grad=False,
    )
    ...
    with PreparationSession(
        ...
        try:
            ...
            torch.testing.assert_close(
                replayed, reference(source), rtol=0.008, atol=1e-5
            )
        ...
        with pytest.raises(ValueError, match="exceeds capacity"):
            layer(
                torch.empty(capacity + 1, hidden, dtype=torch.bfloat16, device=device)
            )
