...
def test_compressor_split_projection_precision_and_replay(ratio):
    ...
    weight = (
        torch.randn(ratio * 512, 5120, dtype=torch.bfloat16, device=device) / 5120**0.5
    )
    ...
    def check(rows):
        for index, output in enumerate(outputs):
            expected = (
                source[:rows].float()
                @ weight[index * 512 : (index + 1) * 512].float().T
            ).to(dtype)
            torch.testing.assert_close(
                output[:rows], expected, rtol=0.008 if ratio == 1 else 0.002, atol=1e-4
            )
            assert (
                torch.isfinite(output[:rows]).all()
                and torch.count_nonzero(output[:rows]) > 0
            )
...
def test_mhc_weighted_collapse_and_mean_use_prepared_plans(monkeypatch):
    ...
    owner = SimpleNamespace(
        ...
        hc_attn_scale=torch.empty(3, device=device),
        hc_ffn_scale=torch.empty(3, device=device),
        hc_attn_base=torch.empty(24, device=device),
        hc_ffn_base=torch.empty(24, device=device),
        attn_norm=SimpleNamespace(
            weight=torch.ones(5120, device=device, dtype=torch.bfloat16)
        ),
        ffn_norm=SimpleNamespace(
            weight=torch.ones(5120, device=device, dtype=torch.bfloat16)
        ),
    )
    workload = B12xWorkload(
    ...
    requests = tuple(
        request for request in unit.requests if ".collapse." in request.name
    )
    ...
    with PreparationSession(
        ...
        for weighted in (False, True):
            ...
            def reference(rows, weighted=weighted):
                if weighted:
                    return (
                        (source[:rows].float() * mix[:rows, :, None]).sum(1).bfloat16()
                    )
                return source[:rows].float().mean(1).bfloat16()

            for rows in (1, 8, 257, 4096):
                actual = module.collapse(
                    source[:rows], mix[:rows] if weighted else None
                )
                torch.testing.assert_close(
                    actual, reference(rows), rtol=0.008, atol=1e-5
                )
            ...
            try:
                ...
                assert (
                    ...
                    and actual.data_ptr() == pointer
                )
                torch.testing.assert_close(
                    actual, reference(4096), rtol=0.008, atol=1e-5
                )
            finally:
                graph.reset()
...
@torch.no_grad()
def test_dspark_context_projection_is_ready_before_pool_preparation(
    ...
    scales = torch.randint(
        123, 128, ((rank + 512) // 32, hidden // 32), dtype=torch.uint8, device=device
    ).view(torch.float8_e8m0fnu)
    fused = SimpleNamespace(weight=weight, weight_scale_inv=scales)
    projection = dspark._ContextKVProjection(
        SimpleNamespace(fused_wqa_wkv=fused, q_lora_rank=rank), 4096
    )
    ...
    for rows in counts:
        ...
        def prime(state, rows=rows):
            ...
            output = torch.empty(
                rows, rank + 512, 1, dtype=torch.bfloat16, device=device
            )
            binding = state.bind(
                scratch=scratch,
                source=source[:rows],
        ...
        reference_requests.append(
            plan.request(name=f"fused-reference.m{rows}", prepare_call=prime)
        )
    with PreparationSession(device=device, autotune=True, compile_workers=2) as session:
        ...
        for rows in (1, 7, 8, 96, 257, 1023, 4096):
            expected_state, expected_binding = references[
                next(count for count in counts if rows <= count)
            ]
        ...
        for rows in (96, 4096):
            ...
            try:
                ...
                torch.testing.assert_close(
                    actual,
                    expected_binding.output[:rows, rank:, 0],
                    rtol=0.008,
                    atol=1e-5,
                )
            finally:
...
def test_weights_collection_prepares_shared_and_markov_logits_heads(
    ...
    with torch.device(device):
        markov = ParallelLMHead(
            32768, 256, params_dtype=torch.bfloat16, disable_tp=True
        )
    ...
    with PreparationSession(device=device, autotune=True, compile_workers=2) as session:
        ...
        for rows in (1, 7, 8):
            ...
            expected_bias = torch.nn.functional.linear(
                markov_source[:rows], markov.weight
            )
