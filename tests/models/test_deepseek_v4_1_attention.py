...
@pytest.mark.parametrize(
    "groups,heads_per_group,rank,hidden", [(1, 1, 128, 128), (2, 8, 1024, 5120)]
)
@torch.no_grad()
def test_wo_preparation_exact_rows_owns_output_and_replays(
    ...
    module._wo_projection_weights = quantize_wo_projection_weights_mxfp8_torch(
        torch.randn(groups, rank, group_width, device=device, dtype=torch.bfloat16)
        / group_width**0.5,
        torch.randn(hidden, groups * rank, device=device, dtype=torch.bfloat16)
        / (groups * rank) ** 0.5,
    )
    ...
    module.swa_cache_layer = SimpleNamespace(
        block_size=256,
        kv_cache=torch.empty(0, dtype=torch.uint8, device=device),
    )
    ...
    units = attention._AttentionHelpers(module).get_b12x_preparation_units(
        module, workload
    )
    ...
    source = (
        torch.randn(
            capacity, module.n_local_heads, 512, device=device, dtype=torch.bfloat16
        )
        / 8
    )
    positions = torch.arange(capacity, device=device, dtype=torch.int64).remainder_(
        table.shape[0]
    )
    with PreparationSession(
        ...
        for rows, is_prefill, plan in cases:
            ...
            binding = wo.bind_inv_rope(
                ...
                cos_sin_cache=table,
                weights=module._wo_projection_weights,
                heads_per_group=heads_per_group,
                nope_dim=448,
                rope_dim=64,
            )
            expected = wo.run_inv_rope(binding=binding, plan=plan).clone()
            actual = module._o_proj(
                source[:rows], positions[:rows], is_prefill=is_prefill
            )
            if is_prefill:
                assert rows not in module._wo_plans
            assert torch.isfinite(actual).all() and torch.count_nonzero(actual) > 0
            ...
            try:
                ...
                assert (
                    torch.isfinite(replayed).all() and torch.count_nonzero(replayed) > 0
                )
            finally:
                graph.reset()
...
def test_indexer_declares_bounded_score_rows_at_model_context_capacity(
    ...
    torch.nn.Module.__init__(module)
    module.prefix = "model.layers.0.attn"
    module.capacity, module.max_model_len, module.swa_width = 4096, context, 128
    ...
    module.swa_cache_layer = SimpleNamespace(
        ...
        kv_cache=torch.empty(
            (1, mla.page_nbytes(256, cache_kind="swa")),
            dtype=torch.uint8,
            device=device,
        ),
    )
    module.kv_cache = torch.empty(
        (1, mla.page_nbytes(128, cache_kind="indexed")),
        dtype=torch.uint8,
        device=device,
    )
    module.indexer = SimpleNamespace(
        ...
        k_cache=SimpleNamespace(
            kv_cache=torch.empty(
                (1, dsa_indexer.index_mxfp4_page_bytes(128)),
                dtype=torch.uint8,
                device=device,
            ),
        ),
    ...
    workload = B12xWorkload(
        stage="state",
        token_counts=(1, 7, 64, 257, 4089, 4096),
        fixed_token_counts=(1, 7, 64),
        output_dtype=torch.bfloat16,
        max_tokens=4096,
        max_seqs=8,
    ...
    for mode, chunk in chunks.items():
        for total in counts:
            for offset in range(0, total, chunk):
                ...
                assert (
                    rows * plan.query.max_page_table_width * plan.query.page_size
                    < 2**31
                )
    assert (
        module._index_plan("prefill", 256).query.max_page_table_width == context // 256
    )
    if compacted:
        assert module._short_index_shape is None
    else:
        short = module._index_plan("prefill_short", 1024).query
        assert short.max_page_table_width * short.page_size == 16384
        assert module._short_index_shape == (1024, 128)
...
def test_indexer_primer_restores_live_cache(layer_id):
    ...
    cache = torch.randint(
        0,
        256,
        (8, dsa_indexer.index_mxfp4_page_bytes(128)),
        dtype=torch.uint8,
        device=device,
    )
    ...
    for mode, rows, width in (
        ...
        requests.append(
            plan.request(
                name=f"indexer.{mode}.{rows}", prepare_call=call, benchmark_call=call
            )
        )
    with PreparationSession(
        device=device, autotune=False, compile_workers=2
