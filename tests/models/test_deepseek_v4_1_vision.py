...
def test_native_small_vision_block_aligner_and_image_isolation():
    ...
    session = PreparationSession(
        device=torch.device("cuda"), autotune=False, compile_workers=2
    )
    units = [
        unit
    ...
    with torch.no_grad():
        ...
        patches = torch.randn(4 * 7 + 1, 3, 2, 2, device="cuda", dtype=torch.bfloat16)[
            1:
        ]
...
def test_vision_linear_prepares_bias_and_replays_into_caller_output(
    ...
    with PreparationSession(
        ...
        session.prepare(
            tuple(
                request
                for unit in _units_from_modules(layer, workload)
        ...
        try:
            ...
            torch.testing.assert_close(
                output, _linear(source, layer), rtol=0.01, atol=0.01
            )
        finally:
            graph.reset()
