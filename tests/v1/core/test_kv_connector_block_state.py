...
def test_connector_block_state_resolves_offered_tables_and_explicit_boundaries() -> (
    None
):
    grouped_ids = (
        [201, 202, 203, 204],
    ...
    manager.get_block_ids.assert_not_called()
    assert state.req_ids == {"req"}
    assert state.get_block_ids("req") == grouped_ids
