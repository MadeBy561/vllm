...
def test_exception_restores_terminal(tty):
    ...
    with (
        pytest.raises(ValueError, match="startup failed"),
        terminal.EscapeKey(lambda: None),
    ):
...
def _executor(monkeypatch):
    ...
    return SimpleNamespace(
        vllm_config=SimpleNamespace(
            kernel_config=SimpleNamespace(enable_b12x_autotune=True)
        ),
        cancel_b12x_autotuning=cancel.set,
...
def test_disabled_warmup_skips_keyboard(monkeypatch, disable):
    ...
    monkeypatch.setattr(
        terminal, "_open_terminal", lambda: pytest.fail("opened terminal")
    )
    with Executor.b12x_warmup_control(executor):
        assert executor._b12x_autotuning_cancel.is_set()
