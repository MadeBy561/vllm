...
def test_capture_restores_terminal_after_failure():
    ...
    try:
        with (
            pytest.raises(ValueError, match="preparation failed"),
            PreparationOutput(lines.append, descriptors=(slave,)),
        ):
...
def test_capture_leaves_pipe_destinations_alone():
    ...
    try:
        with PreparationOutput(
            lambda line: pytest.fail(line), descriptors=(writer,)
        ) as output:
...
def _startup_with_native_output():
    ...
    def worker():
        ...
        def advance(*, cancel_tuning=False):
            handler.emit(
                logging.LogRecord(
                    "worker", logging.INFO, "", 0, "worker-bound-handler", (), None
                )
            )
            os.write(2, b"\n\nworker-native-write\n")
            subprocess.run(
        ...
        try:
            result = WorkerBase.run_b12x_preparation(
                SimpleNamespace(rank=0, advance_b12x_preparation=advance),
                **child.recv(),
            )
            child.send(result)
        finally:
    ...
    def rpc(method, kwargs=None):
        ...
        handler.emit(
            logging.LogRecord(
                "engine", logging.INFO, "", 0, "engine-bound-handler", (), None
            )
        )
        os.write(2, b"\nengine-native-write\n")
        parent.send(kwargs)
    ...
    try:
        Executor._run_b12x_preparation(
            SimpleNamespace(
                collective_rpc=rpc,
...
def _terminal_text(data, rows, columns):
    ...
    class Rect(ctypes.Structure):
        _fields_ = [
            (name, ctypes.c_int)
            for name in ("start_row", "end_row", "start_col", "end_col")
        ]
    ...
    try:
        ...
        count = lib.vterm_screen_get_text(
            screen, buffer, len(buffer), Rect(0, rows, 0, columns)
        )
...
def test_executor_serializes_bound_handlers_native_writes_and_child_output(columns):
    ...
    try:
        ...
        for message in (
            "engine-bound-handler",
            "engine-native-write",
            "worker-bound-handler",
            "worker-native-write",
            "compiler-child-output",
            "worker-final-fragment",
        ):
        ...
        assert all(
            line.startswith("│") and line.endswith("│") for line in panel[1:-1]
        ), screen
    finally:
...
def _loading_with_native_output():
    ...
    summary = dict(
        ranks=4,
        payload_bytes=305_750_000_000,
        shared_physical_bytes=299_595_000_000,
        physical_bytes=299_640_000_000,
        shared_transfer_seconds=0.1,
    )
    group = SimpleNamespace(
        epoch=1,
        progress=None,
        summary=summary,
        start=lambda session: None,
        _gather=lambda value: None,
    )
    session = SimpleNamespace(
        shared_read_group=group, progress=None, flush=lambda: None, stats=lambda **_: {}
    )
    ...
    def route(self, model, config):
        ...
        handler.emit(
            logging.LogRecord(
                "loader", logging.INFO, "", 0, "loading-bound-handler", (), None
            )
        )
        ...
        subprocess.run(
            [
                sys.executable,
                "-c",
                'import os; os.write(1, b"loading-child-output\\n")',
            ],
            check=True,
        )
...
def test_weight_loading_keeps_one_complete_panel_with_native_logs(columns, tmp_path):
    ...
    try:
        ...
        for message in (
            "loading-bound-handler",
            "loading-native-write",
            "loading-child-output",
            "loading-returned",
        ):
            assert screen.count(message) == 1, screen
        top = next(
            i for i, line in enumerate(screen.splitlines()) if line.startswith("┌")
        )
        bottom = next(
            i for i, line in enumerate(screen.splitlines()) if line.startswith("└")
        )
