...
def test_progress_aggregates_work_totals_and_preserves_rank_local_batch_counts() -> (
    None
):
...
def test_progress_does_not_sum_replicated_fixed_candidate_count() -> None:
    ...
    progress = _aggregate_b12x_progress(
        [{"global_rank": rank, "done": False, "progress": fixed} for rank in range(2)]
    )
...
def test_attention_tuning_rendezvous_ignores_rank_local_device_ordinal(variant):
    ...
    for rank in ranks:
        ...
        def metadata(shape, dtype, device=device):
            return FakeTensor(
                mode, torch.empty(shape, device="meta", dtype=dtype), device
            )
...
def test_peer_failure_drains_every_rank_before_returning():
    ...
    assert all(
        item["error"]["message"] == "candidate setup failed" for item in outcomes
    )
...
def test_candidate_progress_waits_for_every_expected_rank_to_finish_planning():
    ...
    known = PreparationProgress(
        False, False, (), False, total_candidates=120, measured_candidates=30
    )
    rank_zero = {"global_rank": 0, "progress": known}
    assert (
        _aggregate_b12x_progress([rank_zero], expected_ranks=(0, 1)).total_candidates
        is None
    )
    rank_one = {"global_rank": 1, "progress": replace(known, total_candidates=None)}
    assert (
        _aggregate_b12x_progress(
            [rank_zero, rank_one], expected_ranks=(0, 1)
        ).total_candidates
        is None
    )
    ...
    outcomes = [
        {
            ...
            "progress": PreparationProgress(
                ...
                tuning_rank=rank,
                candidate_sharded=True,
                measured_candidates=count,
                total_candidates=11901,
            ),
        }
...
def test_cancel_drains_pending_winners_then_orders_collective_warmup():
    ...
    for job in jobs[:2]:
        ...
        def blocked_advance(
            *, collective_key=None, tuning=None, advance=advance, pending=pending
        ):
            if (
                pending[0] is not None
                and collective_key != pending[0].ready_collectives[0].key
            ):
                return pending[0]
...
def test_heuristic_warmup_is_local_on_each_rank(tmp_path, monkeypatch):
    ...
    contract = TuningContract(
        component_id="test.rank_warmup",
        query_schema_version=1,
        config_schema_version=1,
        query_fields=frozenset({"rows"}),
        config_fields=frozenset({"width"}),
        encode_query=lambda q: {"rows": q.rows},
        encode_config=lambda c: {"width": c.width},
        decode_config=lambda value: Config(value["width"]),
        validate_query=lambda *_: None,
        validate_config=lambda *_: None,
        default_config=lambda *_: Config(7),
        knobs=(Knob(name="width", values=(1, 2, 4)),),
        ...
        with PreparationSession(
            ...
            coordinator = B12xPreparationCoordinator(
                ...
                world_group=SimpleNamespace(
                    ranks=(0, 1), tcp_store_group=SimpleNamespace(store=store)
                ),
            )
